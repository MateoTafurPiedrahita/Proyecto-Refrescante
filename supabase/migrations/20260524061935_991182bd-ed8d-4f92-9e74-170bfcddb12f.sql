...
BEGIN
  ...
  VALUES (NEW.id, COALESCE(NEW.raw_user_meta_data->>'nombre_completo', NEW.raw_user_meta_data->>'full_name', ''));
...
CREATE POLICY "Admins can update pedidos" ON public.pedidos FOR UPDATE TO authenticated USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Users can view own pedido items" ON public.pedido_items FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM public.pedidos WHERE pedidos.id = pedido_items.pedido_id AND pedidos.user_id = auth.uid()));
CREATE POLICY "Admins can view all pedido items" ON public.pedido_items FOR SELECT TO authenticated USING (public.has_role(auth.uid(), 'admin'));
CREATE POLICY "Users can create items in own pedidos" ON public.pedido_items FOR INSERT TO authenticated
  WITH CHECK (EXISTS (SELECT 1 FROM public.pedidos WHERE pedidos.id = pedido_items.pedido_id AND pedidos.user_id = auth.uid()));
